...
python3 dataset/generate_gt_mesh.py --output "$OUT/ground_truth.obj" >/dev/null

for N in "${DEPTH_COUNTS[@]}"; do
  echo "=== depth cameras: $N ==="
  DS=data/_sweep_d$N
  CFG=configs/_sweep_d$N.json
  python3 dataset/generate_synth_data.py \
  ...
  CH=$(python3 eval/chamfer.py --recon "$OUT/recon_d$N/frame_00000" --gt "$OUT/ground_truth.obj" --samples 150000 \
