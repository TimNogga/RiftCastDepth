...
// depth_debug[0..19]: diagnostic counters, printed after fusion
...
// Carve band: shallow positive SDF from grazing limb rays must not carve, only deep free space does
constexpr float kPositiveSdfThresholdMeters = 0.01f;
constexpr float kTruncMarginMeters = 0.05f;  // TSDF truncation tau
constexpr float kVetoSaturationCount = 2.0f; // surface votes at which the soft veto saturates
constexpr float kGrazingAirMinMeters = 0.01f; // grazing observers abstain on the surface band only
// Grazing gate: per-pixel depth gradient as incidence proxy (~70deg at this rig); 0 disables
constexpr float kGrazingGradThreshMetersPerPixel = 0.0025f;
...
// Curless-Levoy incidence weighting: w_i = cos(theta)^p from the per-pixel depth gradient
constexpr bool  kUseIncidenceWeight    = true;
constexpr float kIncidenceWeightPower  = 1.0f;
constexpr float kMinIncidenceWeight    = 0.05f;  // near-tangent samples abstain
...
// Bilinear depth at the sub-pixel projection; nearest-neighbour mismatch reads as false free-space
template <typename ValueT>
inline C10_DEVICE float
...
// True if the surface is viewed too obliquely here, judged by the per-pixel depth gradient
template <typename ValueT>
inline C10_DEVICE bool
...
// Curless-Levoy incidence weight w = cos(theta)^p; returns 0 (abstain) at invalid or near-tangent pixels
template <typename ValueT>
inline C10_DEVICE float
                       ...
                       const float fz)
{
    ...
    float gx = 0.5f * (zr - zl);
    float gy = 0.5f * (zd - zu);
    float tan2 = fz * fz * (gx * gx + gy * gy);
    float cos_theta = rsqrtf(1.0f + tan2);
    float w = (kIncidenceWeightPower == 1.0f) ? cos_theta : powf(cos_theta, kIncidenceWeightPower);
    return (w < kMinIncidenceWeight) ? 0.0f : w;
}

// Focal length in pixels from the world->clip transform: fx = 0.5*W*||first-row spatial part||
                       ...
                       int64_t* depth_debug)
{
    ...
    // Experimental octree-level depth carving, disabled: the TSDF field already opens concavities
    ...
    for (auto tid = id; tid < N; tid += num_threads)
    {
        ...
        if (apply_depth_carving && !is_empty && should_refine) {
            ...
            // Space-carving vote: carve only voxels several cameras see as deep free space and none as surface
            constexpr int   kMinAirVotes   = 2;
            constexpr float kShieldMarginM = 0.10f;
            ...
            for (auto batch = int64_t{0}; batch < depths.size(0); ++batch) {
                ...
                // Metric depth: .w for OpenGL clip space, camera-space z for OpenCV
                ...
                // Edge shield: skip views where the voxel projects onto a depth discontinuity
                          ...
                          int64_t* depth_debug)
{
    ...
    // Octree depth carving disabled; depth is handled by the TSDF fusion kernels
    bool apply_depth_carving = false;
                     ...
                     const float concavity_forward_offset_meters)
{
    ...
    for (auto tid = id; tid < N; tid += num_threads)
    {
        ...
        // sdf > 0: free space in front of the measured surface, ~0: on it, < 0: behind it
        float sdf = (z_sensor - kDepthSurfaceSlackMeters) - (metric_z - concavity_forward_offset_meters);

        constexpr float kTruncMargin = 0.05f;
        // Occlusion gate: a camera whose measured surface lies in front of the voxel cannot see it -> no vote
        if (sdf < -kPositiveSdfThresholdMeters) {
            atomicAdd(reinterpret_cast<unsigned long long*>(&depth_debug[17]), 1ULL);
            continue;
        }

        // Incidence-weighted fusion: grazing samples are down-weighted, zero weight abstains
        float w_inc;
        ...
        atomicAdd(&sparse_tsdf[tid], w_inc * tsdf);
        atomicAdd(&sparse_weights[tid], w_inc);
        if (sdf > kPositiveSdfThresholdMeters)
        {
            atomicAdd(&sparse_positive_weights[tid], w_inc);  // carve vote
        }
        else
        {
            atomicAdd(&sparse_solid_weights[tid], w_inc);     // surface vote
        }
                     ...
                     const int64_t N)
{
    ...
    for (int64_t i = tid; i < N; i += gridDim.x * blockDim.x) {
        ...
        if (w > 0.0f) {
            ...
            float surface_w  = sparse_solid_weights[i];

            // Soft veto: blend the carved TSDF zero-crossing with the hull by the head-on surface votes, min-fused
            float mean_sdf  = sparse_tsdf[i] / w;
            float depth_val = 0.5f - 0.5f * (mean_sdf / kTruncMarginMeters);   // air 0, surface 0.5, solid 1
            depth_val = fminf(fmaxf(depth_val, 0.0f), 1.0f);
            float p = fminf(surface_w / kVetoSaturationCount, 1.0f);
            float fused = depth_val * (1.0f - p) + vh * p;
            sparse_vh[i] = fminf(vh, fused);
                                       ...
                                       const int depth_fusion_preset)
{
    ...
    // RIFT_CARVE_OFFSET env var overrides the carve offset at runtime
